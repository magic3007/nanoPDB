...
# NanoPDB V1: dynamic breakpoints (line breakpoint)
class NanoPDB:
    def __init__(self):
    ...
    def _breakpoint(
        ...
        @add_helper
        def break_at_line(line: int):
            file = frame.f_code.co_filename
            self.add_breakpoint(file, line)

        @add_helper
        def break_at_file_line(file: str, line: int):
            self.add_breakpoint(file, line)
    ...
    def _should_break_at(self, frame: types.FrameType):
        ...
        if (
            p in self._breakpoints_in_files
            and frame.f_lineno in self._breakpoints_in_files[p]
        ):
            return True
        return False
